...
//		print 'lol';
	...
	public function AddSignature($parameters, &$object, &$action, $hookmanager)
	{
		...
		if ($this->isJpsunProContractOnlineSignature($object, $sourcefile)) {
	...
	/**
	 * Check whether the signed contract is generated by the JPSUN PRO model.
	 *
	 * @param CommonObject $object Contract object
	 * @param string $sourcefile Source PDF path
	 * @return bool
	 */
	private function isJpsunProContractOnlineSignature($object, $sourcefile)
	{
		if (is_object($object) && !empty($object->model_pdf) && strtolower((string) $object->model_pdf) === 'jpsunpro') {
			return true;
		}
