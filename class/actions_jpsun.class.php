...
//		print 'lol';
	...
	public function AddSignature($parameters, &$object, &$action, $hookmanager)
	{
		...
		if ($this->isJpsunContractWithDedicatedClientSignature($object, $sourcefile)) {
			return $this->addJpsunProClientOnlineSignature($sourcefile, $newpdffilename, $sigpath, $online_sign_name, $langs, $object);
		}
	...
	/**
	 * Check whether the signed contract is generated by a JPSUN model with a dedicated customer signature box.
	 *
	 * @param CommonObject $object Contract object
	 * @param string $sourcefile Source PDF path
	 * @return bool
	 */
	private function isJpsunContractWithDedicatedClientSignature($object, $sourcefile)
	{
		if (is_object($object) && !empty($object->model_pdf) && in_array(strtolower((string) $object->model_pdf), array('jpsunpro', 'soleilaquitain'), true)) {
			return true;
		}
		...
		return ($annexfile !== null && dol_is_file($annexfile));
	}

	/**
	 * Add the online customer signature in the JPSUN PRO customer signature box.
	 *
	 * @param string $sourcefile Source PDF path
	 ...
	 */
	private function addJpsunProClientOnlineSignature($sourcefile, $newpdffilename, $sigpath, $onlineSignName, $langs, &$object)
	{
		try {
			$pdf = pdf_getInstance();
			if (class_exists('TCPDF')) {
			...
			for ($i = 1; $i <= $pagecount; $i++) {
				...
				if ($i === (int) $pagecount) {
					$layout = jpsunPdfJpsunProSignatureLayout((float) $s['w'], (float) $s['h']);
			...
			dol_syslog('JPSUN PRO online signature failed: '.$e->getMessage(), LOG_ERR);
