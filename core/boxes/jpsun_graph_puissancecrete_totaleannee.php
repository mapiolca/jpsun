...
class jpsun_graph_puissancecrete_totaleannee extends ModeleBoxes
{
	...
	private function fetchTotalYear($db, $year)
	{
		...
		$sql = "SELECT SUM(COALESCE(pef.jpsun_pc_install, 0)) as total FROM ".MAIN_DB_PREFIX."commande as p LEFT JOIN ".MAIN_DB_PREFIX."commande_extrafields as pef ON pef.fk_object = p.rowid WHERE p.fk_statut > 0 AND p.entity IN (".$entitySql.") AND p.date_livraison IS NOT NULL AND p.date_livraison >= '".$db->idate(dol_get_first_day($year, 1, false))."' AND p.date_livraison <= '".$db->idate(dol_get_last_day($year, 12, false))."'";
		$resql = $db->query($sql);
