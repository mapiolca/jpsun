 ...
 */
class pdf_jpsun extends ModelePDFContract
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = 'JPSUN';
		$this->description = $langs->trans("StandardContractsTemplatebyJpsun");
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$outputlangs->loadLangs(array("main", "dict", "companies", "contracts", "compta", "jpsun@jpsun"));
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if ($valide > 0 || $object->specimen) {
						...
						if (!getDolGlobalString('CONTRACT_HIDE_QTY_ON_PDF')) {
						    $unit = $objectligne->getLabelOfUnit('long', $langs) ;//pdf_getlineunit($object, $i, $outputlangs, $hidedetails);
							$txt .= $outputlangs->transnoentities("Quantity") .' ('. $unit . ') : <strong>' . $objectligne->qty . ' </strong>';
						}
						if (!getDolGlobalString('CONTRACT_HIDE_PRICE_ON_PDF')) {
							$txt .= ' - ' . $outputlangs->transnoentities("UnitPriceHT") . ' : <strong>' . price($objectligne->subprice) . ' '. $outputlangs->transnoentitiesnoconv("Currency".$currency).'</strong>';
						}
