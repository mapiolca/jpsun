 ...
 */
class pdf_jpsun_stock extends ModelePDFStock
{
	/**
	 * @var int
	 */
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->stock->dir_output) {
			...
			if ($object->specimen) {
				...
				$file = $dir."/".$objectref." - ".date("Y-m-d h-i-s").".pdf";
			}
			...
			if (file_exists($dir)) {
				...
				if ($resql) {
					...
					for ($i = 0; $i < $nblines; $i++) {
						...
						// Auto height for row: use the tallest cell (ref/label)
						$lineH = 3;
						$wRef = $this->wref;
						$wLabel = $this->posxqty - $this->posxlabel - 0.8;
						$txtRef = $outputlangs->convToOutputCharset($objp->ref);
						$txtLabel = $outputlangs->convToOutputCharset($objp->produit);
						if (method_exists($pdf, 'getStringHeight')) {
							$hRef = $pdf->getStringHeight($wRef, $txtRef, false, true, '', 1);
							$hLabel = $pdf->getStringHeight($wLabel, $txtLabel, false, true, '', 1);
						} else {
							// Fallback for FPDF-like engines
							$hRef = $lineH;
							$hLabel = $lineH;
							if ($wRef > 0) {
								$hRef = max($lineH, ceil($pdf->GetStringWidth($txtRef) / $wRef) * $lineH);
							}
							if ($wLabel > 0) {
								$hLabel = max($lineH, ceil($pdf->GetStringWidth($txtLabel) / $wLabel) * $lineH);
							}
						}
						$hRow = max($lineH, $hRef, $hLabel);

						$pdf->startTransaction();
						$pdf->writeHTMLCell($this->wref, $hRow, $curX, $curY, $txtRef, 0, 1, false, true, 'J', true);
						...
						// Enforce next Y using computed row height
						$nexY = $curY + $hRow;
						...
						$pdf->MultiCell($this->posxqty - $this->posxlabel - 0.8, $hRow, $txtLabel, 0, 'L', false, 0, '', '', true, 0, false, true, $hRow, 'T', true);
