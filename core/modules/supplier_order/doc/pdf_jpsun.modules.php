...
/**
 *	\file       htdocs/core/modules/supplier_order/doc/pdf_jpsun.modules.php
 *	\ingroup    fournisseur
 *	\brief      File of class to generate suppliers orders from jpsun model
 */
 ...
 */
class pdf_jpsun extends ModelePDFSuppliersOrders
{
	/**
	...
	public function __construct($db)
	{
		...
		$langs->loadLangs(array("main", "bills", "jpsun@jpsun"));
		...
		if (getDolGlobalInt('PRODUCT_USE_UNITS')) {
			$this->posxtva = 120;// Par défaut : 95
			$this->posxup = 120;// Par défaut : 114
			$this->posxqty = 138;// Par défaut : 132
			$this->posxunit = 151;// Par défaut : 147
		} else {
			$this->posxtva = 106;// Par défaut : 106
			$this->posxup = 122;// Par défaut : 122
			$this->posxqty = 145;// Par défaut : 122
			$this->posxunit = 162;// Par défaut : 162
		}
	...
	public function write_file($object, $outputlangs = null, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		// Load translation files required by the page
		$outputlangs->loadLangs(array("main", "orders", "companies", "bills", "dict", "products", "jpsun@jpsun"));
		...
		if ($conf->fournisseur->commande->dir_output) {
			...
			if ($object->specimen) {
			    ...
			    $objectnamesupplier = dol_sanitizeFileName($object->thirdparty->name);
				$objectref = dol_sanitizeFileName($object->ref);
				$objectrefsupplier = dol_sanitizeFileName($object->ref_supplier);
				$objectproject = dol_sanitizeFileName($object->project->title);
				$dir = $conf->fournisseur->commande->dir_output.'/'.$objectref;
				
				/*
				if(empty($object->project)){    //Ancienne version de nom de fichier.
				    $file = $dir."/".$objectnamesupplier." - ".$outputlangs->transnoentities("Order")." ".$objectref.".pdf";
				}else{
				    $file = $dir."/".$objectnamesupplier." - ".$outputlangs->transnoentities("Order")." ".$objectref." - ".$objectproject.".pdf";
				}
				*/
				if(empty($object->project)){
				    $file = $dir."/".$objectref." - ".$outputlangs->transnoentities("Order")." ".$objectnamesupplier.".pdf";
				}else{
				    $file = $dir."/".$objectref." - ".$outputlangs->transnoentities("Order")." ".$objectnamesupplier." - ".$objectproject.".pdf";
				}
...
**/
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		// Load translation files required by the page
		$outputlangs->loadLangs(array("main", "orders", "companies", "bills", "sendings", "jpsun@jpsun"));
		...
		if (!empty($object->date_commande)) {
			...
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($w, 5, $outputlangs->transnoentities("OrderDate")." : ".dol_print_date($object->date_commande, "day", false, $outputlangs, true), '', 'R');
		}
		...
		if (!empty($object->array_options['options_jpsun_date_de_livraison_souhaitee'])) {
			$posy += 4;
			$pdf->SetXY($posx - 90, $posy);
			$pdf->MultiCell(190, 3, $outputlangs->transnoentities("jpsun_date_de_livraison_souhaitee")." : ".dol_print_date($object->array_options['options_jpsun_date_de_livraison_souhaitee'], $usehourmin, false, $outputlangs, true), '', 'R');
		}
