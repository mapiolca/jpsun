...
require_once '../class/enedis.class.php';

// Load translation files required by the page
$langs->load("projects");
$langs->load("jpsun@jpsun");
$langs->load("enedis@jpsun");
...
$projet_enedis = (isModEnabled('agenda') && isModEnabled('jpsun') && ($user->hasRight('agenda', 'myactions', 'read') || $user->hasRight('agenda', 'allactions', 'read'))) ? ' '.$langs->trans("projet_enedis") : '';
$title = $langs->trans('Events').$projet_enedis.' - '.$object->ref.' '.$object->name;
...
print dol_get_fiche_head($head, 'projet_enedis', $langs->trans("Project"), -1, ($object->public ? 'projectpub' : 'project'));
...
$titlelist = $langs->trans("ProjectContact");
//$titlelist = '';

//print_barre_liste($titlelist, 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', 0, -1, '', 0, $morehtmlright, '', 0, 1, 0);
print_barre_liste($titlelist, 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', '', '', 'fa-address-book', 0, '', '', '', 0, 0, 1);
    
// Contacts lines (modules that overwrite templates must declare this into descriptor)
	$dirtpls = array_merge($conf->modules_parts['tpl'], array('/custom/jpsun/tpl'));
...
/**
	...
	$messagingUrl = DOL_URL_ROOT.'/custom/jpsun/tabs/projet_enedis.php?id='.$object->id;
...
**/
	...
	if (isModEnabled('agenda')) {
		...
		$morehtmlright .= dolGetButtonTitle($langs->trans('AddAction'), '', 'fa fa-plus-circle', DOL_URL_ROOT.'/comm/action/card.php?action=create'.$out.'&backtopage='.urlencode($_SERVER["PHP_SELF"].'?id='.$object->id), '', $addActionBtnRight);
	}
	...
	$titlelist = $langs->trans("jpsun_ddr_suivi_enedis");
	//$titlelist = '';

	//print_barre_liste($titlelist, 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', 0, -1, '', 0, $morehtmlright, '', 0, 1, 0);

    print_barre_liste($titlelist, 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', '', '', 'fa-plug', 0, $morehtmlright, '', '', 0, 0, 1);
	
	$actioncode = '';
	$search_code = '';
	$filters = array();
	$search_code = 'DDR';
	$filters['search_type'] = $actioncode;
	$filters['search_agenda_label'] = $search_agenda_label;
	$filters['search_code'] = $search_code;
	$filters['search_rowid'] = $search_rowid;
    
	show_actions_enedis_messaging($conf, $langs, $db, $object, null, 0, $actioncode, '', $filters, 'a.datep,a.id', 'DESC');
}
